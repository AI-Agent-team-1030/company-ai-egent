...
**社内ナレッジ検索くん**は、企業向けの社内AIアシスタントプラットフォームです。社内ドキュメントをAIに学習させ、従業員がチャット形式で質問すると、社内情報に基づいた正確な回答を得られます。
...
| 機能 | 説明 |
|------|------|
| マルチプロバイダー対応 | Gemini、Claude、GPTから選択（各回答に使用モデルを表示） |
| ナレッジ検索 | 社内ドキュメントを自動検索して回答 |
| Googleドライブ検索 | ドライブ内のファイルも検索対象 |
| 会話履歴保存 | 過去の会話を保存・参照可能 |
| 引用表示 | 回答完了後にソース別（📄社内ナレッジ / 📁Googleドライブ）でアニメーション表示 |
| タイピングエフェクト | 回答が段階的に表示、完了後に引用が一気に表示 |
| 回答再生成 | 同じ質問で別の回答を生成 |
| Shift+Enter改行 | 複数行の質問が可能（入力欄が自動でリサイズ） |
| 重複送信防止 | 連続クリックや連続Enterによる二重送信を防止 |
...
#### 基本的な使い方
1. チャットページを開く
2. 質問を入力（複数行の場合はShift+Enterで改行、入力欄は自動で広がります）
3. Enterで送信
4. AIの回答がタイピングエフェクトで表示
5. 回答完了後、引用情報がアニメーションで表示（ソース別に色分け）
   - 📄 緑色：社内ナレッジ
   - 📁 青色：Googleドライブ
6. 「開く ↗」リンクで元ファイルを直接開ける

#### ナレッジ検索のON/OFF
- 画面下部のトグルで切り替え
- ON: 社内ドキュメント+ドライブを検索して回答（トグル横に「+ Drive」表示）
- OFF: 一般的な知識のみで回答
...
3. 選択したモデルで回答が生成され、回答欄にモデル名が表示される
4. 同じ会話内でもモデルを切り替え可能（切り替え後の質問から新しいモデルが適用）
...
#### Q: 社内ナレッジ検索くんとは何ですか？
**A:** 社内ナレッジ検索くんは、企業向けの社内AIアシスタントです。社内ドキュメントをアップロードすると、AIがその内容を学習し、従業員の質問に対して社内情報に基づいた回答を返します。
...
#### Q: 長文の質問を入力したい
**A:** Shift+Enterで改行できます。入力欄は内容に応じて自動で広がります（最大200px）。

#### Q: 会話履歴は保存されますか？
**A:** はい、自動的にFirestoreに保存されます。サイドバーから過去の会話を参照できます。

#### Q: モデルを切り替えたのに、回答がGeminiと表示されます
**A:** 以前はナレッジ検索時に自動でGeminiが使用されていましたが、現在は修正済みです。選択したモデルで回答が生成され、回答欄に正しいモデル名が表示されます。

#### Q: 引用情報はいつ表示されますか？
**A:** AIの回答がすべて表示された後、引用情報がアニメーションで一気に表示されます。ソース別に色分けされています（緑色：社内ナレッジ、青色：Googleドライブ）。

#### Q: 同じ質問が2回送信されてしまいます
**A:** 重複送信防止機能があるため、通常は発生しません。問題が続く場合は、ページを再読み込みしてください。
...
*このドキュメントは社内ナレッジ検索くん Version 2.0に基づいています。*
